...
const MenuItemPopUp = ({
  ...
  const addToCartPrice = calCartProductPrice({
    menuItem,
    selectedSize,
    selectedExtras,
  });
  ...
  function handleSelectExtras(
    ...
    if (checked) {
      ...
      setSelectedExtras((prev) => {
        return prev.filter((item) => item.name !== extraIngredient.name);
      });
    }
  }
    ...
    >
      ...
      >
        ...
        >
          ...
          <h2 className="font-bold text-center my-3 text-primary">
            {menuItem.name}
          </h2>
          ...
          <button
            ...
            onClick={() => {
              onAdd(menuItem, selectedSize, selectedExtras);
              console.log(menuItem, selectedSize, selectedExtras);
            }}
          >
            Add to Cart{" "}
            <span className="font-semibold">TK. {addToCartPrice}</span>
          </button>
          <Button
            color="danger"
            variant="flat"
